...
1. BROADCAST - 直播模式
2. PK - PK对战模式
3. VIDEO - 录像模式 (循环播放直播画面/视频)
4. MUSIC - 歌曲模式
5. REPLAY - 回放模式 (播放录播文件，支持点播)
6. OTHER - 其他模式
...
class ModeManager:
    ...
    async def auto_switch_for_song_request(self, queue_count: int) -> None:
        """有点歌时自动切换到歌曲模式，队列清空时回到录像"""
        if queue_count > 0:
            await self.set_mode(Mode.MUSIC, f"队列有 {queue_count} 首歌")
            await self.update_mode_state(Mode.MUSIC, queue_count=queue_count)
        elif self.current_mode == Mode.MUSIC:
            await self.set_mode(Mode.VIDEO, "歌曲队列已清空")

    async def auto_switch_for_pk(self, is_pk_active: bool, opponent_name: str = "") -> None:
        """PK开始时切换到PK模式，结束时回到录像"""
        if is_pk_active:
            ...
            if self.current_mode == Mode.PK:
                await self.set_mode(Mode.VIDEO, "PK已结束")
    ...
    async def auto_switch_for_broadcast(self, is_live: bool, viewer_count: int = 0, uptime_seconds: int = 0) -> None:
        ...
        if is_live:
            ...
            if self.current_mode == Mode.BROADCAST:
                await self.set_mode(Mode.VIDEO, "直播已结束")
    ...
    def get_mode_by_command(self, command: str) -> Optional[Mode]:
        ...
        支持命令: "直播模式", "PK模式", "歌曲模式", "录像模式", "回放模式", "其他模式"
        """
