...
分辨率: 1920×1080 (16:9)
A区位置: x=0, y=0, 大小: 1344×756
B区位置: x=1385, y=45, 大小: 520×435
C区位置: x=1385, y=485, 大小: 520×595
背景: 跨越全屏 (为边角留白和视觉平衡)
```

### 2.2 各区域详解

#### **A 区 - 主内容区（1344×756, 16:9）**
...
**背景**：使用 background.png，跨越整个画布，为各区留白

```
OBS 配置:
1. MScreen 中添加图像源: background.png
2. 位置: x=0, y=0
3. 大小: 1920×1080 (全屏)
4. 顺序: 放在最底层（在所有源之下）
```

**框架遮罩**：可选，用 frame-overlay.png 美化边界

```
OBS 配置:
1. MScreen 中添加图像源: frame-overlay.png
2. 位置: x=0, y=0
3. 大小: 1920×1080 (全屏)
4. 顺序: 放在最顶层（在所有源之上）
```
